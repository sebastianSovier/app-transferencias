...
export class TransferenciasService {
  ...
  ObtenerHistorialMovimientos(requestMovimiento:any) {
    const headers = new HttpHeaders({'Authorization':'Bearer '+sessionStorage.getItem('token'),'Access-Control-Allow-Origin': '*'});
    const result: Observable<any> = this.http.post(AppConfig.settings.UrlApi + '/Movimientos/ObtenerMovimientos',requestMovimiento,{headers: headers});
    return result;
  }
